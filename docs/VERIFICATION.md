...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 810/811 grün (1 übersprungen) — `scripts/pruefstand.sh`, Commit ddb1db4, 2026-08-13 |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 316/316 grün — `scripts/pruefstand.sh`, Commit ddb1db4, 2026-08-13 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 18/18 grün — `scripts/pruefstand.sh`, Commit ddb1db4, 2026-08-13 |
