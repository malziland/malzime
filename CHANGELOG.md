...
**Qualitäts-Hotfix für die Single-Large-Call-Pipeline.** Der erste Live-Test mit aktiviertem `useSingleLargeCall`-Flag (Männer-Profil, Bikepacking-Event) zeigte deutliche Qualitäts-Regressionen im Vergleich zur 3-Call-Pipeline: Beast war zu zahm (nur ~75 Wörter statt ~150, Karten nur 7-11 Wörter statt 15-25), `ad_targeting` lieferte generische Branchen („Outdoor-Ausrüstung") statt konkreter Marken („Garmin Edge 1040"), zentrale Live-Bausteine wie der 8-Kategorien-Charakter-Katalog, `AGE_ANCHOR` mit FORCED MAPPING für Erwachsene und `GENDER_ANCHOR` fehlten komplett. Ursache: der `singleLargePrompt` in `functions/src/locales/de/prompts.js` und `en/prompts.js` war als verkürzter Forschungs-Prototyp aus `functions/scripts/single-large-call-test.js` 1:1 ins Locale übernommen worden, statt aus den bewährten Live-Bausteinen (`systemNormal`, `systemBoost`, `AGE_ANCHOR`, `GENDER_ANCHOR`) restrukturiert zu werden.

### Behoben

- **`singleLargePrompt` als Composition aus Live-Bausteinen statt eigenständige Re-Implementation:** der Prompt wird jetzt nach `module.exports` per Template-Literal aus den existierenden `systemNormal` + `systemBoost` + `AGE_ANCHOR` + `GENDER_ANCHOR` zusammengebaut. Single-Source-of-Truth — Verbesserungen an einem Live-Baustein wirken automatisch auf BEIDE Pipelines (3-Call + Single-Large). Damit kommen alle bewährten Live-Anweisungen zurück: der 8-Kategorien-Charakter-Katalog (Standard + Beast), die FORCED-MAPPING-Altersregel (Nasolabialfalten → MINIMUM 38 Jahre), die Gender-First-Aus-Gesichtsmerkmalen-Regel, die Manipulation-Trigger-Liste mit 20+ Optionen, das KEINE-Preisangaben-Verbot.
- **Beast-Profil-Schärfe wiederhergestellt:** Beast-`profileText` jetzt 10-12 Sätze, ~150 Wörter (vorher 6-8 Sätze, ~100 Wörter — entspricht jetzt der Live-Vorgabe in `systemBoost`). Pflicht: mindestens 2 unangenehme Wahrheiten mit konkretem Bildbeleg im selben Satz, korporativ-kalter Ton („Wir wissen, dass du...", „Wir verkaufen dir...").
- **Beast-Karten-Länge wiederhergestellt:** Beast-Karten jetzt 15-25 Wörter, zwei zynische Sätze (Klassifikation + Spitze mit Bildbeleg) statt 12-Wort-Stichpunkten. Beispiel im Schema: „Du bist Leistungsfanatiker mit chronischer Unsicherheit. Die zwanghafte Event-Teilnahme zeigt: Bestätigung holst du nur über Outdoor-Quälerei."
- **Konkrete Marken statt generischer Branchen:** `ad_targeting`-Anweisung wortgenau aus dem Live-`mistralDescribeAddendum` übernommen — „KONKRETE Marken/Produkte aus sichtbaren Logos UND ableitbarem Lifestyle, KEINE generischen Branchen wie 'Outdoor-Ausrüstung'". Mit Beispielen: „Garmin Edge 1040", „Rapha Pro Team", „Apple Watch Ultra".
- **Manipulation-Trigger-Vielfalt:** Trigger-Pool aus `systemBoost` mit 20+ Optionen jetzt im Single-Large-Prompt verfügbar (über die Komposition). Hinweis im Schema: „NICHT 4× FOMO oder 4× Statusangst".

### Geändert

- **`functions/scripts/single-large-call-test.js` nutzt jetzt den Live-Prompt:** statt einer eigenen Prompt-Kopie wird `require("../src/locales/de/prompts").singleLargePrompt` direkt geladen. Damit testet das Forschungs-Script IMMER gegen denselben Prompt wie Production — Drift wie bei v2.1.1 kann strukturell nicht mehr passieren.
- **Reale Token- und Latenz-Werte mit dem restrukturierten Prompt (1 Bild, Live-API-Messung vor Deploy):** 11.956 Tokens pro Analyse (vorher v2.1.1 mit Pfusch-Prompt: 5.700), 47,6 s Latenz (vorher v2.1.1: ~30 s). Vergleich zur heutigen 3-Call-Pipeline: **-44 % Tokens** (statt der ursprünglich versprochenen -73 %), aber **+25 % langsamere Einzel-Latenz** (47 s vs 38 s). Hard-Facts identisch in beiden Modi, Beast-Profil deutlich substanzieller (226 Wörter statt 75), konkrete Marken (Nike Dri-FIT, Garmin Forerunner, Strava Premium) statt generischer Branchen, vielfältige Trigger.
- **Architektur-Wert verschiebt sich:** der primäre Gewinn der Single-Large-Architektur ist NICHT mehr die Einzel-Analyse-Latenz, sondern die **vollständige Befreiung vom 2603-TPM-Bottleneck**. Alle Tokens landen im Large 2512 (2M TPM statt 100K) — Workshop-Concurrency kann auf 10+ ohne 429-Risiko. Das macht den Unterschied bei einem 40er-Workshop spürbar: heute wartet der letzte Teilnehmer ~5 Min (Concurrency 3), mit Single-Large + Concurrency 10 wären es ~90 s pro Job × ⌈40/10⌉ = ~6 min total für die ganze Klasse statt ~13 min (rechnerisch — Lasttest steht aus).

### Lehre (intern, im Memory als bleibende Regel persistiert)

- **Bei Architektur-Erweiterungen, die einen neuen Prompt-Pfad einführen:** NIEMALS einen neuen Prompt von Grund auf schreiben. IMMER die bewährten Live-Bausteine wiederverwenden, auch wenn das technisch aufwendiger ist. Forschungs-Prototypen mit verkürzten Prompts sind OK für „funktioniert die Architektur" — aber nie 1:1 in Production-Code übernehmen. Refactor zu Live-Bausteinen ist Pflicht, nicht optional. Ein Prompt-Pfusch hier hätte die Workshop-Qualität nach Aktivierung deutlich verschlechtert und im schlimmsten Fall mitten in einem laufenden Schul-Workshop sichtbar werden können.
