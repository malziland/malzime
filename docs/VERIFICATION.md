...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 1389/1389 grün — `scripts/pruefstand.sh`, Commit 76f290c, 2026-09-08 (ohne installiertes `gcloud` werden die Tests übersprungen, die die gcloud-Parameter der Rollback-Skripte abgleichen) |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 665/665 grün — `scripts/pruefstand.sh`, Commit 76f290c, 2026-09-08 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 362/362 grün — `scripts/pruefstand.sh`, Commit 76f290c, 2026-09-08 |
