...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 1353/1353 grün — `scripts/pruefstand.sh`, Commit 694f659, 2026-09-01 (ohne installiertes `gcloud` werden die Tests übersprungen, die die gcloud-Parameter der Rollback-Skripte abgleichen) |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 483/483 grün — `scripts/pruefstand.sh`, Commit 694f659, 2026-09-01 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 330/330 grün — `scripts/pruefstand.sh`, Commit 694f659, 2026-09-01 |
