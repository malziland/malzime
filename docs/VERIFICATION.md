...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 857/857 grün — `scripts/pruefstand.sh`, Commit 2fa8e76, 2026-08-19 |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 447/447 grün — `scripts/pruefstand.sh`, Commit 2fa8e76, 2026-08-19 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 258/258 grün — `scripts/pruefstand.sh`, Commit 2fa8e76, 2026-08-19 |
