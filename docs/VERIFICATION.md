...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 1316/1316 grün — `scripts/pruefstand.sh`, Commit d66c65c, 2026-09-11 |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 805/805 grün — `scripts/pruefstand.sh`, Commit d66c65c, 2026-09-11 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 373/373 grün — `scripts/pruefstand.sh`, Commit d66c65c, 2026-09-11 |
...
| Stundenzähler: jeder eingelassene Auftrag genau einmal im Fenster, auch wenn der Zähler unter Andrang aufs Netz ausweicht | `functions/src/__tests__/zaehler-genau-einmal.test.js` (Transaktion schreibt nie / spät / im Augenblick des Zeitlimits; Freigabe trifft den eigenen Eintrag) und Simulator: `firebase emulators:exec --only functions,firestore "node functions/scripts/stundenzaehler-emulator-messung.js 30"`. Ungültig, sobald `counter.js` Einlass, Nachtrag oder Freigabe ändert | ✅ Simulator, 30 gleichzeitig: 29 Netz-Fälle, 30 von 30 genau einmal, 0 doppelt, 0 fehlend — 2026-09-11 (Arbeitsbaum auf 1c96577) |
...
| Betriebs-Rollback ohne Deploy (Feature-Flags, Wartungsmodus) | Firestore `featureFlags/current`, `config/betriebsprofil`, `config/maintenance`; Verfahren in [RUNBOOK.md](RUNBOOK.md) | ✅ produktiv erprobt (Architektur-Umstellungen v2.0/v2.2 über Flags; Wartungsmodus bei jedem Deploy) |
