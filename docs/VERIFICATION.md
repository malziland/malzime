...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 1335/1335 grün — `scripts/pruefstand.sh`, Commit 9259f44, 2026-09-11 |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 805/805 grün — `scripts/pruefstand.sh`, Commit 9259f44, 2026-09-11 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 373/373 grün — `scripts/pruefstand.sh`, Commit 9259f44, 2026-09-11 |
