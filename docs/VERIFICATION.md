...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 1483/1483 grün — `scripts/pruefstand.sh`, Commit 4f59f9c, 2026-09-18 |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 823/823 grün — `scripts/pruefstand.sh`, Commit 4f59f9c, 2026-09-18 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 373/373 grün — `scripts/pruefstand.sh`, Commit 4f59f9c, 2026-09-18 |
