...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 1345/1345 grün — `scripts/pruefstand.sh`, Commit 0dfeeb3, 2026-09-16 |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 818/818 grün — `scripts/pruefstand.sh`, Commit 0dfeeb3, 2026-09-16 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 373/373 grün — `scripts/pruefstand.sh`, Commit 0dfeeb3, 2026-09-16 |
