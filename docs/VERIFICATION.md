...
|---|---|---|
| Backend-Unit-Tests | CI-Job `test-backend` (jeder Push/PR); lokal `npm test --prefix functions` | ✅ 857/858 grün (1 übersprungen) — `scripts/pruefstand.sh`, Commit 3b3a18b, 2026-08-19 |
| Frontend-Unit-Tests | CI-Job `test-frontend`; lokal `npm run test:frontend` | ✅ 427/427 grün — `scripts/pruefstand.sh`, Commit 3b3a18b, 2026-08-19 |
| E2E kritischster Nutzerfluss (Demo-Foto → Queue → Disclaimer → Profil) | CI-Job `test-e2e` (Playwright, Container-Image = Paketversion); lokal `npm run test:e2e` | ✅ 233/233 grün — `scripts/pruefstand.sh`, Commit 3b3a18b, 2026-08-19 |
